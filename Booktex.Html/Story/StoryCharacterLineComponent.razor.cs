...
public partial class StoryCharacterLineComponent
{
    ...
    private static MarkupString ProduceRollingRs(BookCharacterLinePart part)
    {
        ...
        var returnee = $@"<div class=""animation-text"">{replacedText}</div>";
        return new MarkupString(returnee);
    }


    private static readonly string FastRollingR = @"<div class=""rolling-r""><div class=""rolling-r-fast"">r</div></div>";
    private static readonly string SlowRollingR = @"<div class=""rolling-r""><div class=""rolling-r-slow"">r</div></div>";
